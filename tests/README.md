...
| `test_auction_backend.py` | Live reverse auctions, rolled back: launch rules, accept/decline, the two-bidder start rule, ranking and ties, the minimum decrement, the soft-close extension, what a vendor may see (never a rival's name), read-only bid log, close, award repricing the request, the PO going to the winner at the winning price, Bid Again, the cron, and every auction view. |
| `test_vendor_scope.py` | A supplier login reads the orders placed with its own company and nobody else's — items and timeline included — a supplier with none sees none, a rival's order is refused, the buyer still sees everything, and the ownership check the PO acknowledgment relies on agrees. Rolled back. |
